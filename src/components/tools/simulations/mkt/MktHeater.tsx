  ...
  return (
    <div className="mx-auto w-full max-w-md">
      <div className="flex flex-col items-center">
        <div
          className="relative flex h-12 w-12 items-end justify-center overflow-hidden rounded-xl border border-white/10 bg-black/50 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] lg:h-10 lg:w-10"
          aria-hidden
        >
          {heat > 0.02 && (
            <div
              className="absolute bottom-0 left-1/2 w-8 -translate-x-1/2"
              style={{ height: `${14 + heat * 28}px` }}
            >
              <div className="absolute inset-x-1.5 bottom-0 h-full rounded-t-full bg-orange-500/90" />
              <div className="absolute inset-x-2.5 bottom-0 h-[70%] rounded-t-full bg-amber-300" />
              <div className="absolute inset-x-3.5 bottom-0 h-[40%] rounded-t-full bg-white/80" />
            </div>
          )}
          {cold > 0.02 && (
            <div
              className="absolute inset-x-1 bottom-1 flex items-end justify-center gap-0.5"
              style={{ height: `${10 + cold * 30}px` }}
            >
              {Array.from({ length: Math.max(1, Math.round(1 + cold * 4)) }).map(
                (_, index) => (
                  <span
                    key={index}
                    className="w-2 rounded-sm border border-sky-200/70 bg-gradient-to-b from-sky-50 to-sky-300/80"
      ...
      <label className="mt-1.5 block">
        ...
        <input
          ...
          className="simulation-range h-9 w-full cursor-pointer touch-manipulation lg:h-7"
          style={{
            background:
