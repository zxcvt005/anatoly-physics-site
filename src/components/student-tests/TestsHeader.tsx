...
type TestsHeaderProps = {
  token: string;
};

export function TestsHeader({ token }: TestsHeaderProps) {
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-black/70 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <nav className="flex items-center gap-1 sm:gap-2" aria-label="Навигация тестов">
          <Link
            href={`/student/${token}`}
