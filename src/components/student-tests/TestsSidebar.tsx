...
import {
  isTestsNavItemActive,
  type TestsNavItem,
} from '@/lib/tests/student-navigation';

type TestsSidebarProps = {
...
function SidebarNavItem({
  ...
  const isExactMatch =
    isTestsNavItemActive(item.href, pathname) &&
    (hasChildren
      ? pathname === hrefPath || pathname.startsWith(`${hrefPath}#`)
      : true);
  const isParentOfActive =
    hasChildren &&
    item.children!.some((child) => isTestsNavItemActive(child.href, pathname));
...
function SidebarContent({
  ...
  const { navigation, loading } = useTestsData();

  if (loading) {
    return <p className="px-3 py-2 text-sm text-zinc-500">Загрузка...</p>;
  }

  return (
    <nav className="space-y-1" aria-label="Разделы тестов">
      {navigation.map((item) => (
        <SidebarNavItem
          key={item.id}
          item={item}
...
export function TestsSidebar({ isMobileOpen, onMobileClose }: TestsSidebarProps) {
...
export function TestsSidebarToggle({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
